...
const submissionPayloadSchema = {
  type: 'object',
  properties: {
    kind: { type: 'string', enum: ['inline_utf8', 'arweave_tx'] },
    value: { type: 'string', minLength: 1 }
  },
  required: ['kind', 'value'],
  additionalProperties: false
...
const submissionContextSchema = {
  type: 'object',
  properties: {
    client: { type: 'string', enum: ['web', 'cli', 'api'] },
...
const submissionSchema = {
  type: 'object',
  properties: {
    type: { const: 'wofi.submission.v1' },
    schema_version: { const: '1.0' },
    payload: submissionPayloadSchema,
    payload_hash: { type: 'string', pattern: CONTENT_ID_PATTERN },
    mime_type: { type: 'string', minLength: 1 },
    context: { ...submissionContextSchema, nullable: true },
    ...baseProperties
  },
  required: ['type', 'schema_version', 'payload', 'payload_hash', 'mime_type', 'created_at'],
  additionalProperties: false
}
...
export const schemaMap: Record<string, Record<string, object>> = {
  ...
  'wofi.submission.v1': { '1.0': submissionSchema },
