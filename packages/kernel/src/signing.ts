...
export function normalizePubkey(input: string | Uint8Array): string {
...
type Signed<T> = T & {
  author: { kind: 'pubkey'; value: string }
  signature: { alg: 'ed25519'; value: string }
  content_id: string
}

export async function signObject<T extends Record<string, any>>(
  obj: T,
  ...
  if (!ed.etc.sha512Sync) {
    ed.etc.sha512Sync = (msg: Uint8Array) => createHash('sha512').update(msg).digest()
  }
...
export async function generateKeypair(): Promise<{ publicKey: string; privateKey: Uint8Array }> {
